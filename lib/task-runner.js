...
class TaskRunner extends EventEmitter {
	...
	createTask(name, dependencies, taskFunction) {
			...
			if (deps instanceof Set) {
				...
				return function (args) {
					...
					return promise.then((argsList) => {
						...
						return argsList.reduce((acc, argsSource) => {
							return acc.merge(argsSource);
						}, seed);
					});
				};
			}

			// If not a Set, `deps` must be an Array. In this case, order does matter, so we
			// execute in series.
		...
		function wrappedTask(args) {
			...
			return executeDependencies(args).then((newArgs) => {
				let res;
				try {
					res = taskFunction(newArgs);
				} catch (err) {
